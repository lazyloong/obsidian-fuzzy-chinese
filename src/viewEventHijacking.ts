...
export function hijackingEmptyView(plugin: ThePlugin) {
  ...
  plugin.registerEvent(
    app.workspace.on('active-leaf-change', (leaf) => {
      ...
      let hotkey = app.hotkeyManager.printHotkeyForCommand('fuzzy-chinese-pinyin:open-search');
    ...
    app.workspace.on('active-leaf-change', (leaf) => {
      ...
      plugin.registerDomEvent(valueEl, 'mouseup', async (e) => {
        ...
        containerEl.addEventListener('click', (e) => {
          if (e.target instanceof HTMLElement && e.target.className == 'multi-select-container')
            newInputEl.focus();
        });
          ...
          plugin.tagEditorSuggest
            ...
            .filter(
              (p) =>
                ...
                (rendered.entry.value && !rendered.entry.value.includes(p.item.name))
            );
