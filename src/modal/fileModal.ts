...
export default class FileModal extends FuzzyModal<Item> {
  ...
  getEmptyInputSuggestions(): MatchData[] {
    ...
    if (this.tags.length == 0) {
      items = this.app.workspace
        .getRecentFiles()
        .map((p) => this.index.items.find((q) => q.type === FileItemType.file && q.path === p));
    } else {
    ...
    if (query[0] == ' ' && this.plugin.settings.file.quicklySelectHistoryFiles) {
      ...
      switch (query.length) {
        case 1:
          break;
        case 2:
          let index = max(['asdfjklgh'.indexOf(query[1]), '1234567890'.indexOf(query[1])]);
  ...
  renderSuggestion(matchData: MatchData, el: HTMLElement) {
    ...
    if (matchData.history && this.plugin.settings.file.quicklySelectHistoryFiles) {
      ...
      auxEl.createEl('kbd', {
        ...
        text: this.plugin.settings.file.quicklySelectHistoryFilesHint[
          this.getEmptyInputSuggestions().findIndex((p) => p.item.path == matchData.item.path)
        ],
      });
    }
    ...
    } else if (matchData.item.type === FileItemType.unresolvedLink) renderer.addIcon('file-plus');
  }
...
class PinyinIndex extends PI<Item> {
  ...
  initEvent() {
    this.registerEvent(
      this.metadataCache.on('changed', (file, _, cache) => this.update('changed', file, cache))
    );
    ...
    switch (type) {
      case 'changed': {
        ...
        this.aliasItems = this.aliasItems
          ...
          .concat(aliasItem);
        this.linkItems = this.linkItems.filter((item) => item.path !== file.path).concat(linkItem);
        break;
      }
      case 'create': {
